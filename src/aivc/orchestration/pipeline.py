...
def run_integrated_pipeline(
    ...
    resolved = resolve_client(settings, company_name)
    ...
    parent_run_id = create_pipeline_run(
        settings,
        client,
        requested_options={"company": company_name},
        component_versions={
    ...
    try:
        ...
        with ThreadPoolExecutor(
            ...
            citation_future = executor.submit(
                generate_company_report, settings, resolved.canonical_name
            )
        ...
        recon_state, persistence = run_recon(
            prepared_recon,
            citation_bundle,
            deliver=shared_settings.aivc_delivery_mode == "configured",
        )
